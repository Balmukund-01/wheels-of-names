...
let items = ["Abhishek", "Sagar", "Amar", "Nitesh", "Sonu", "Ibrar", "Avinash", "Ravi"];
...
function assignColorsWithoutRepeats(items, colorPalette) {
    ...
    if (assignedColors.length > 1 && assignedColors[0] === assignedColors[assignedColors.length - 1]) {
        const temp = assignedColors[assignedColors.length - 2];
        assignedColors[assignedColors.length - 2] = assignedColors[assignedColors.length - 1];
