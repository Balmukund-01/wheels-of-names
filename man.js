...
nameEditor.addEventListener("focus", () => (isEditing = true));
nameEditor.addEventListener("blur", () => {
  isEditing = false;
  // Trigger wheel redraw on blur
  updateItemsFromEditor();
});
...
// Touch and click spin (only if not typing)
...
function updateItemsFromEditor() {
  const lines = Array.from(nameEditor.children)
...
// Live update from #nameEditor to wheel
document.getElementById("nameEditor").addEventListener("input", () => {
  // Get new names from editor
  const lines = Array.from(document.getElementById("nameEditor").children)
    .map((div) => div.textContent.trim())
    .filter((name) => name.length > 0);
